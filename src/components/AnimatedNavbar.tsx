...
export default function AnimatedNavbar({ showAnimation = false, isVisible = true }: AnimatedNavbarProps) {
  if (!isVisible) return null
  
  return (
    <motion.header
      className="sticky top-0 z-50 w-full border-b border-[var(--theme-primary)]/20 bg-[var(--theme-primary)] shadow-sm overflow-visible"
      initial={showAnimation ? { y: -100, opacity: 0 } : false}
    ...
    >
      <div className="container flex h-14 max-w-screen-2xl items-center justify-between overflow-visible">
        ...
        <motion.div
          initial={showAnimation ? { y: -50, opacity: 0 } : false}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.6, delay: showAnimation ? 1 : 0 }}
        >
          <NavigationMenu className="text-[var(--theme-text)] overflow-visible">
            <NavigationMenuList>
              <NavigationMenuItem>
                <NavigationMenuTrigger className="bg-[var(--theme-primary)] text-[var(--theme-text)] hover:bg-[var(--theme-primary)]/80 focus:bg-[var(--theme-primary)]/80 data-[active]:bg-[var(--theme-primary)]/90 data-[state=open]:bg-[var(--theme-primary)]/90">
                  Getting started
                </NavigationMenuTrigger>
                  ...
                  <ul className="grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr] bg-white">
                    <li className="row-span-3">
                      <NavigationMenuLink asChild>
                        ...
                        >
                          <div
                            className="mb-2 mt-4 text-lg font-medium text-[var(--theme-primary)]"
              ...
              <NavigationMenuItem>
                <NavigationMenuTrigger className="bg-transparent text-[var(--theme-text)] hover:bg-[var(--theme-text)]/10 focus:bg-[var(--theme-text)]/10 data-[active]:bg-[var(--theme-text)]/10 data-[state=open]:bg-[var(--theme-text)]/10">
                  Services
                </NavigationMenuTrigger>
                <NavigationMenuContent className="bg-white border border-gray-200">
        ...
        >
          <Link href="/" className="flex items-center space-x-2">
            <Image src="/logo.png" alt="RehabEase Logo" width={32} height={32} className="object-contain" />
            <span
              className="font-bold text-2xl text-[var(--theme-text)]"
              style={{ fontFamily: "var(--theme-font-heading)" }}
            >
              RehabEase
            </span>
        ...
        <motion.div
          initial={showAnimation ? { y: -50, opacity: 0 } : false}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.6, delay: showAnimation ? 1.2 : 0 }}
        >
          <NavigationMenu className="text-[var(--theme-text)] overflow-visible">
            <NavigationMenuList>
              <NavigationMenuItem>
                <NavigationMenuLink asChild>
                  <Link
                    ...
                    className={cn(
                      navigationMenuTriggerStyle(),
                      "bg-transparent text-[var(--theme-text)] hover:bg-[var(--theme-text)]/10"
                    )}
                  >
                    Appointments
                  </Link>
    ...
    return (
      <li>
        <NavigationMenuLink asChild>
          ...
          >
            <div
              className="text-sm font-medium leading-none text-[var(--theme-primary)]"
              style={{ fontFamily: "var(--theme-font-heading)" }}
            >
              {title}
            </div>
